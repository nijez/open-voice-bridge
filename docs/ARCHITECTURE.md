...
DJI 官方资料说明接收器可以通过 USB-C 连接电脑；发射器直接蓝牙连接的官方兼容范围没有把桌面系统列为通用直连目标，因此项目没有把产品带蓝牙直接等同于桌面麦克风支持。随后在真实 Windows 11 主机上采集到 `DJI-MIC2-*` 蓝牙设备、标准 Hands-Free/HFP 服务和录音端点证据。实现仍以“系统录音端点当前可用”为通过条件：仅已配对不算成功，也不复用 RC003 的 BLE GATT/ATVV 协议。

macOS 通过 CoreAudio 枚举用户可选输入，识别 DJI Mic 2 后仍由用户明确选择；Windows 通过 PortAudio 枚举录音端点，并在设置窗口中提供 RC003 / DJI Mic 2 明确设备选择。选择 DJI 后只显示系统录音输入与硬件控制边界，不启动 RC003 BLE/HID/ATVV 桥，也不把发射器的录音/连接/电源键伪装成 Windows 可映射按键。两端都不修改系统默认麦克风、不保存音频内容。当前状态为开发/真机验收中，不能在连续录音门通过前标为完整支持。
